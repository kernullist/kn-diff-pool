...
2. A `src\kn-diff\x64\Debug\kn-diff.sys` found by walking upward from the current directory or executable directory.
3. A `kn-diff.sys` found by walking upward from the current directory or executable directory.
4. Fallback: `src\kn-diff\x64\Debug\kn-diff.sys` under the current directory.
...
By default, exports are written under an `exports` directory in the current working directory.
...
2. 현재 디렉터리 또는 실행 파일 디렉터리에서 위로 올라가며 찾은 `src\kn-diff\x64\Debug\kn-diff.sys`.
3. 현재 디렉터리 또는 실행 파일 디렉터리에서 위로 올라가며 찾은 `kn-diff.sys`.
4. fallback: 현재 디렉터리 아래 `src\kn-diff\x64\Debug\kn-diff.sys`.
...
기본 export 위치는 현재 작업 디렉터리의 `exports` 폴더입니다.
