...
malware나 game cheat는 자신의 실행 코드 또는 보조 코드를 커널 pool에 올려두는 경우가 있습니다. 이 코드가 일정 크기 이상이면 작은 pool이 아니라 Big Pool 영역에 위치하는 경우가 많습니다.

따라서 분석 샌드박스 환경에서는 malware 또는 game cheat를 실행하기 전에 baseline Big Pool 스냅샷을 생성하고, 대상을 실행한 뒤 다시 current 스냅샷을 생성해서 diff를 확인하는 방식이 유용합니다. 새로 나타난 Big Pool allocation을 중심으로 보면 커널 pool에 숨겨진 malware/game cheat 코드를 찾는 범위를 크게 줄일 수 있고, 이후 dump/search/PTE 분석으로 이어가기 쉬워집니다.
